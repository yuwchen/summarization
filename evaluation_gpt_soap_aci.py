...
result_root = './Results/test1ChatGPT_.csv'
#result_root = './Results/test1GPT-4_.csv'

outputdir = './Evaluation_scores/aci_chatgpt'
gt_df = pd.read_csv('./data/aci-bench/clinicalnlp_taskB_test1.csv')
gpt_df = pd.read_csv(result_root)
...
for idx, row in gt_df.iterrows():
    ...
    pred_note = gpt_df[gpt_df['encounter_id'] == encounter_id]['note'].to_list()
    references.append(gt_note) 
    predictions.append(remove_symbol(pred_note[0]))

    pred_note = process_text(remove_symbol(pred_note[0]))
